...
const PersonalDetailScreen = ({ navigation }) => {
  ...
  const handleCameraCapture = async () => {
    try {
      const permissionResult = await launchCamera({
        mediaTypes: 'photo',
        maxWidth: 1000,
        maxHeight: 1000,
        quality: 0.8,
      ...
      if (!permissionResult.cancelled && permissionResult.assets && permissionResult.assets.length > 0) {
        handleUploadAvatar(permissionResult.assets[0]);
      }
    } catch (error) {
