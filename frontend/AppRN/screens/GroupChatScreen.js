...
const GroupChatScreen = () => {
      ...
      if (type === 'image') {
        ...
        if (!result.didCancel && result.assets && result.assets.length > 0) {
          const newFiles = result.assets.map((asset) => ({
            ...
            name: asset.fileName || `video_${Date.now()}_${Math.random().toString(36).substr(2, 9)}.mp4`,
  ...
  const downloadFile = async (url) => {
    try {
      // Yêu cầu quyền ghi bộ nhớ (Android)
      let granted = true;
      ...
      if (res.statusCode === 200) {
        Alert.alert("Thành công", `File đã được tải về: ${downloadDest}`);
      } else {
